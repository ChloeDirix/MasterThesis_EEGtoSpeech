...
from DLModel.models.contrastive_cnn_model import ContrastiveAADModel_CNN
...
class AADLightningModule(pl.LightningModule):
    def __init__(self, cfg, input_dims):
        ...
        model_architecture=self.dl_cfg["model"]["architecture"]

        if model_architecture=="w_transformer":
            self.model = ContrastiveAADModel(
                eeg_input_dim=C_eeg,
                stim_input_dim=C_stim,
                d_model=self.dl_cfg["model"]["eeg_encoder"]["d_model"],
                temperature=self.dl_cfg["model"]["contrastive"]["temperature"],
            )

        if model_architecture=="wo_transformer":
    ...
    def training_step(self, batch, batch_idx):
        ...
        eeg, stim, att = batch
        logits = self.model(eeg, stim)
        loss = self.model.contrastive_loss(logits, att)
