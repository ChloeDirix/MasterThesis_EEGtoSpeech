...
from SelectiveAAD.datasets import AADDataset
from SelectiveAAD.plot_checks import PlotCallback
from SelectiveAAD.LightningModule import AADLightningModule
...
def build_dataloaders(cfg, dl_cfg, train_subjects, val_subjects):
    ...
    if dl_cfg.get("quick_test", False):
...
def train_single_fold(cfg, dl_cfg, train_subjects, val_subjects, fold_name="single"):
    ...
    fold_dir = os.path.join(paths.RESULTS_DL, f"fold_{fold_name}")
...
def main():
    ...
    use_loso = dl_cfg.get("use_loso", False)
    ...
    if use_loso:
        ...
        # --------------------------
        train_subjs = cfg["train_subjects"]
        val_subjs = cfg["val_subjects"]
