...
def construct_bpfilter_equiripple(fs, hp, lp, numtaps=513):
    """
    Approximate MATLAB construct_bpfilter() using Parks-McClellan (remez).

    hp, lp in Hz. fs is the design sampling rate (here: 128 Hz).
    """
...
def erbspacebw(f_low, f_high, spacing):
    """Replicates MATLAB erbspacebw."""
    freqs = []
    f = f_low
    while f < f_high:
        bw = spacing * (24.7 * (4.37e-3 * f + 1))
        freqs.append(f)
        f += bw
...
def extract_envelope_das2019(audio, fs_audio, target_fs=32, hp_cutoff=1, lp_cutoff=9, plot=True):
    ...
    # 3) Powerlaw "envelopes": |subband|^power  (NO Hilbert)
    ...
    # 6) Downsample 128 Hz -> target_fs using DECIMATION (no extra LP)
    if fs_env % target_fs != 0:
        raise ValueError(f"target_fs ({target_fs}) must divide {fs_env} exactly (as in MATLAB).")
