...
def preprocess_eeg(eeg, fs, target_fs, band, plot_steps=False, plot_seconds=30):

    # --- Sanity check ---
    if not isinstance(eeg, np.ndarray) or eeg.ndim != 2:
        raise ValueError(f"Invalid EEG array: {type(eeg)}, shape={np.shape(eeg)}")

    n_samples, n_channels = eeg.shape
    print(f"Preprocessing EEG: {n_samples} samples, {n_channels} channels, fs={fs} Hz")
